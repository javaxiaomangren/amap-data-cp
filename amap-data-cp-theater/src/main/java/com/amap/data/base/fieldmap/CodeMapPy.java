...
public class CodeMapPy extends FieldMap {
	...
	static {
		...
		// 市
		String sql;
		sql = "SELECT citycode code,cityname name FROM m_map_city_py";
		DBDataReader ddr = new DBDataReader(sql);
		cityList = ddr.readAll();

		// 区
		sql = "SELECT countycode code,countyname name FROM m_map_county_py";
	...
	public CodeMapPy(TempletConfig templet) {
		type = "城市code+经纬度映射";

		code_map = templet.getString("code_mappy");
	...
	public boolean fieldmap(Map from, Map to) {
		...
		Double lon = from.get(xyMap.get("x")) == null ? null : ObjectUtil
				.toDouble(from.get(xyMap.get("x")));
		Double lat = from.get(xyMap.get("y")) == null ? null : ObjectUtil
