 ...
 * 2013-5-13
 */
package com.amap.data.save.dianping.shopping;
...
public class DianpingShoppingMain {
	private final static Logger log = LoggerFactory
			.getLogger(DianpingShoppingMain.class);
	...
	public void run() throws Exception {
		...
		if (error_status.equals("run")) {
			...
			for (int i = 0; i < 3; i++) {
				try {
					new DianpingSave().dealSave("shopping_dianping_api", true);
				} catch (Exception e) {
					log.info(e.toString());
					flag = false;
				}
