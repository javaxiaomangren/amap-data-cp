 ...
 * 2013-5-13
 */
package com.amap.data.save.dianping.building;
...
public class DianpingBuildingMain {
	private final static Logger log = LoggerFactory
			.getLogger(DianpingBuildingMain.class);
	...
	public void run() throws Exception {
		...
		if (error_status.equals("run")) {
			...
			for (int i = 0; i < 3; i++) {
				try {
					new DianpingSave().dealSave("building_dianping_api", false);
				} catch (Exception e) {
					log.info(e.toString());
					flag = false;
				}
