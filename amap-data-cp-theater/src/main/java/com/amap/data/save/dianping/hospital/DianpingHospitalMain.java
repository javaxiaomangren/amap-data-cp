 ...
 * 2013-5-13
 */
package com.amap.data.save.dianping.hospital;
...
public class DianpingHospitalMain {
	private final static Logger log = LoggerFactory
			.getLogger(DianpingHospitalMain.class);
	...
	public void run() throws Exception {
		...
		if (error_status.equals("run")) {
			...
			for (int i = 0; i < 3; i++) {
				try {
					new DianpingSave().dealSave("hospital_dianping_api", true);
				} catch (Exception e) {
					log.info(e.toString());
					flag = false;
				}
