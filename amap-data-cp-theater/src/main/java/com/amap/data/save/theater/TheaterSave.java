...
public class TheaterSave extends Save{
	...
	protected List<Map> getValidRti(Map rti) {
		...
		for (Map rtiMap : rtiMaps) {
			Map perform = (Map)JSON.parse(SaveHelper
					.getTranserJson(rtiMap.get("performinfo").toString()));
			Object status = perform.get("s");
