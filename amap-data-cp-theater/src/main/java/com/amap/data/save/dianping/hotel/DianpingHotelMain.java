 ...
 * 2013-5-13
 */
package com.amap.data.save.dianping.hotel;
...
public class DianpingHotelMain {
	private final static Logger log = LoggerFactory
			.getLogger(DianpingHotelMain.class);
	...
	public void run() throws Exception {
		...
		if (error_status.equals("run")) {
			...
			for (int i = 0; i < 3; i++) {
				try {
					new DianpingSave().dealSave("hotel_dianping_api", true);
				} catch (Exception e) {
					log.info(e.toString());
					flag = false;
				}
