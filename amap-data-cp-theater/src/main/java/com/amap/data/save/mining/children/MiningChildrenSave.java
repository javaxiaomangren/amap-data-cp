 ...
 * 2013-5-23
 */
package com.amap.data.save.mining.children;
...
public class MiningChildrenSave extends Save{
	@Override
	...
	protected String getCombineJson(Object poiid, List<Map> ids, String cp,
		...
		if(assertValid(deep)){
			...
			combineMap = combineSpec(combineMap, deep, cp);
		}
		return JSON.serialize(combineMap);
	}
	
	@SuppressWarnings({ "rawtypes", "unchecked" })
	protected LinkedHashMap combineSpec(LinkedHashMap combineMap, Map deep,
			String cp) {
		if(deep.get("deep") != null && !deep.get("deep").equals("")){
			Map deepinfo = (Map) JSON.parse(deep.get("deep").toString());
			Object spec = deepinfo.get("mining_children");
			
			Map shape = new HashMap();
			shape.put(cp, spec);

			combineMap.put("spec", shape);
		}
		return combineMap;
	}
