 ...
 * 2013-5-13
 */
package com.amap.data.save.dianping.gasstation;
...
public class DianpingGasstationMain {
	private final static Logger log = LoggerFactory
			.getLogger(DianpingGasstationMain.class);
	...
	public void run() throws Exception {
		...
		if (error_status.equals("run")) {
			...
			for (int i = 0; i < 3; i++) {
				try {
					new DianpingSave().dealSave("gasstation_dianping_api", true);
				} catch (Exception e) {
					log.info(e.toString());
					flag = false;
				}
