 ...
 * 2013-5-13
 */
package com.amap.data.save.dianping.theater;
...
public class DianpingTheaterMain {
	private final static Logger log = LoggerFactory
			.getLogger(DianpingTheaterMain.class);
	...
	public void run() throws Exception {
		...
		if (error_status.equals("run")) {
			...
			for (int i = 0; i < 3; i++) {
				try {
					new DianpingSave().dealSave("theater_dianping_api", false);
				} catch (Exception e) {
					log.info(e.toString());
					flag = false;
				}
