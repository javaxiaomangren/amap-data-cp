...
public class CtripWirelessSaveMain {
	private final static Logger log = LoggerFactory.getLogger(CtripWirelessSaveMain.class);
	...
	public void run() throws Exception {
		...
		if (error_status.equals("run")) {
			...
			for (int i = 0; i < 3; i++) {
				try {
					new CtripWirelessSave().dealSave(
							"hotel_ctrip_wireless_api", true);
				} catch (Exception e) {
					log.info(e.toString());
					flag = false;
				}
	...
	public static void main(String[] args) {
		...
		try {
			new CtripWirelessSave().dealSave("hotel_ctrip_wireless_api", true);
		} catch (Exception e) {
			log.info(e.toString());
		}
	}
